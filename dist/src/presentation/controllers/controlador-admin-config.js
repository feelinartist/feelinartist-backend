...
exports.ControladorAdminConfig = void 0;
const prisma_config_repository_1 = require("../../infrastructure/repositories/prisma-config-repository");
const repositorioConfig = new prisma_config_repository_1.RepositorioConfigPrisma();
class ControladorAdminConfig {
    // REDES SOCIALES
    async listarRedesSociales(req, res) {
        try {
    ...
    async crearRedSocial(req, res) {
        try {
            const { nombre, urlBase, icono } = req.body;
            if (!nombre || !urlBase)
                return res.status(400).json({ message: 'Nombre y URL base requeridos' });
    ...
    async actualizarRedSocial(req, res) {
        try {
            const { id } = req.params;
            const data = req.body;
    ...
    async eliminarRedSocial(req, res) {
        try {
            const { id } = req.params;
    ...
    // METODOS DONACION
    async listarMetodosDonacion(req, res) {
        try {
    ...
    async crearMetodoDonacion(req, res) {
        try {
    ...
    async actualizarMetodoDonacion(req, res) {
        try {
            const { id } = req.params;
            const data = req.body;
    ...
    async eliminarMetodoDonacion(req, res) {
        try {
            const { id } = req.params;
...
exports.ControladorAdminConfig = ControladorAdminConfig;
