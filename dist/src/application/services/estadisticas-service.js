...
class EstadisticasService {
     ...
     */
    async obtenerEstadisticasArtista(perfilArtistaId) {
        ...
        songStats.forEach(stat => {
            ...
            if (stat.genero && (stat.estado === 'ACEPTADO' || stat.estado === 'RECHAZADO')) {
                ...
                generos.forEach(g => {
                    const norm = g.trim();
                    if (norm)
                        generoConteo.set(norm, (generoConteo.get(norm) || 0) + count);
                });
            }
            // Lógica de canciones
     ...
     */
    async obtenerDetalleCancionesArtista(perfilArtistaId, page = 1, limit = 20, search = '', ordenarPor = 'pedidas') {
        ...
        if (search) {
            ...
            allSongs = allSongs.filter(s => s.titulo?.toLowerCase().includes(searchLower) ||
     ...
     */
    async obtenerDetalleCancionesEvento(eventoId, page = 1, limit = 20, search = '', ordenarPor = 'pedidas') {
        const whereClause = {
            eventoId: eventoId
        };
        const songStats = await prisma.pedidoCancion.groupBy({
